...
Чтобы добавить Allure-отчёт переходим к блоку `Post-build Actions`, выбираем кликаем на `Add post-build action` и в выпадающем списке кликаем на `Allure Report`. В поле `Path` проверяем, чтобы путь указывал на правильную директорию в проекте, а именно на `allure-results`. 
...
и в файле `conftest.py` прописываем:
...
В появившемся окне в поле `File` вводим `.env`, а в поле `Content` прописываем переменные из файла `.env`. 
Обязательно указываем отметки в `Create at Workspace` и `Overwrite file`.

Блок с `.env` всегда должен быть выше блока с `Execute shell`.
Нажимаем `Save` для сохранения настроек.
