...
При запуске теста в консоли видно, что сначала вызывается фикстура `browser`, которая печатает `Браузер!`, затем вызывается тест, и после теста вызывается снова фикстура `browser`, которая печатает `Закрываем браузер!`.
