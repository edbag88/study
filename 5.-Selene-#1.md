...
Ниже преведен пример загрузки файла если есть тег `input` с атрибутом `type="file"`:
...
Если нужно переместить переместить ползунок в слайдере, то нужно использовать метод `drag_and_drop_by_offset` из `command`:
```python
from selene import browser, command
